...
WebInspector.DataGrid._preferredWidthSymbol=Symbol("preferredWidth");WebInspector.DataGrid.prototype={_innerAddColumn:function(column,position)
{var columnIdentifier=column.identifier=column.id||String(Object.keys(this._columns).length);if(columnIdentifier in this._columns)
this._innerRemoveColumn(columnIdentifier);if(position===undefined)
position=this._columnsArray.length;this._columnsArray.splice(position,0,column);this._columns[columnIdentifier]=column;if(column.disclosure)
...
if(column.sortable){cell.addEventListener("click",this._clickInHeaderCell.bind(this),false);cell.classList.add("sortable");cell.createChild("div","sort-order-icon-container").createChild("div","sort-order-icon");}},addColumn:function(column,position)
{this._innerAddColumn(column,position);},_innerRemoveColumn:function(columnIdentifier)
{var column=this._columns[columnIdentifier];if(!column)
return;delete this._columns[columnIdentifier];var index=this._columnsArray.findIndex(columnConfig=>columnConfig.identifier===columnIdentifier);this._columnsArray.splice(index,1);var cell=this._headerTableHeaders[columnIdentifier];if(cell.parentElement)
cell.parentElement.removeChild(cell);delete this._headerTableHeaders[columnIdentifier];},removeColumn:function(columnIdentifier)
{this._innerRemoveColumn(columnIdentifier);},setCellClass:function(cellClass)
...
this.selectedNode.parent.collapse();}}else if(event.key==="ArrowRight"){if(!this.selectedNode.revealed){this.selectedNode.reveal();handled=true;}else if(this.selectedNode.hasChildren){handled=true;if(this.selectedNode.expanded){nextSelectedNode=this.selectedNode.children[0];handled=nextSelectedNode?true:false;}else{if(event.altKey)
...
this._dialog.detach();this._delegate.selectItemWithQuery(selectedIndex,this._value());},_itemsLoaded:function()
...
{this._prompt.setText(query);this._prompt.autoCompleteSoon(true);this._scheduleFilter();},_autocomplete:function(proxyElement,wordRange,force,completionsReadyCallback)
{var completions=wordRange.startOffset===0?[this._delegate.autocomplete(wordRange.toString())]:[];completionsReadyCallback.call(null,completions);this._autocompletedForTests();},_autocompletedForTests:function()
...
this._dialog.detach();this._delegate.selectItemWithQuery(itemElement._index,this._value());},itemCount:function()
...
{var panelExtensions=self.runtime.extensions(WebInspector.Panel);for(var extension of panelExtensions)
this._commands.push(WebInspector.CommandMenu.createRevealPanelCommand(extension));var drawerExtensions=self.runtime.extensions("view");for(var extension of drawerExtensions){if(extension.descriptor()["location"]!=="drawer-view")
continue;this._commands.push(WebInspector.CommandMenu.createRevealDrawerCommand(extension));}
...
{var panelName=extension.descriptor()["name"];var tags=extension.descriptor()["tags"]||"";return WebInspector.CommandMenu.createCommand(WebInspector.UIString("Panel"),tags,WebInspector.UIString("Show %s",extension.title()),"",executeHandler,availableHandler);function availableHandler()
...
{var drawerId=extension.descriptor()["id"];var executeHandler=WebInspector.viewManager.showView.bind(WebInspector.viewManager,drawerId);var tags=extension.descriptor()["tags"]||"";return WebInspector.CommandMenu.createCommand(WebInspector.UIString("Drawer"),tags,WebInspector.UIString("Show %s",extension.title()),"",executeHandler);}
...
WebInspector.FlameChart.Events={EntrySelected:Symbol("EntrySelected")}
...
{this.hideHighlight();},elementsToRestoreScrollPositionsFor:function()
{return[this._vScrollElement];},highlightEntry:function(entryIndex)
...
{this._totalHeight=this._levelToHeight(this._dataProvider.maxStackDepth());this._vScrollContent.style.height=this._totalHeight+"px";if(this._scrollTop+this._offsetHeight>this._totalHeight){this._scrollTop=Math.max(0,this._totalHeight-this._offsetHeight);this._vScrollElement.scrollTop=this._scrollTop;}},onResize:function()
...
{this._vScrollElement.scrollTop=0;this._scrollTop=0;this._highlightedMarkerIndex=-1;this._highlightedEntryIndex=-1;this._selectedEntryIndex=-1;this._rangeSelectionStart=0;this._rangeSelectionEnd=0;this._textWidth=new Map();this.update();},_enabled:function()
...
WebInspector.OverviewGrid.Events={WindowChanged:Symbol("WindowChanged"),Click:Symbol("Click")}
...
WebInspector.TimelineOverviewPane.Events={WindowChanged:Symbol("WindowChanged")};WebInspector.TimelineOverviewPane.prototype={_getPopoverAnchor:function(element,event)
